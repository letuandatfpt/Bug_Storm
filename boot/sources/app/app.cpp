...
static bool application_vector_is_valid();
...
int boot_main() {
	...
	***************************************************************************/
	...
	else if (application_vector_is_valid()) {
		APP_PRINT("[BOOT] start application without boot metadata\n");
		jump_to_application_before_reset_peripheral();
	}
...
static bool application_vector_is_valid() {
	...
	uint32_t app_reset_handler = *((uint32_t*)(APP_START_ADDR + sizeof(uint32_t)));

	if ((app_stack_pointer & 0x2FFE0000) != 0x20000000) {
		return false;
	}

	if (app_reset_handler < APP_START_ADDR || app_reset_handler >= 0x08020000) {
		return false;
	}
